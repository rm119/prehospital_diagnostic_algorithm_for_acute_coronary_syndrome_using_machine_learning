...
class Data():
    ...
    # target column
    def set_target(self):
        self.data['ACS'] = self.data['clinical_disease'].isin([1,2]).astype(int)
        self.data['AMI'] = self.data['clinical_disease'].isin([1]).astype(int)
        self.data['STEMI'] = ((self.data['clinical_disease']==1)&(self.data['myocardial_infarction_type']==1)).astype(int)
    ...
    def replace_unknown(self):
        for i in range(1,19):
            self.data[f'interview{i}_unconfirmed'] = (self.data[f'interview{i}']==0).astype(int)
            if i < 18:
                self.data[f'interview{i}_unknown'] = (self.data[f'interview{i}']==1).astype(int)
            self.data.loc[self.data[f'interview{i}'].isin([0]), f'interview{i}'] = np.nan
        for i in range(1, 11):
            self.data[f'medical_history{i}_unknown'] = (self.data[f'medical_history{i}']==1).astype(int)
            self.data[f'medical_history{i}_unconfirmed'] = (self.data[f'medical_history{i}']==0).astype(int)
            self.data.loc[self.data[f'medical_history{i}'].isin([0,1]), f'medical_history{i}'] = np.nan
    ...
    def correct_bpmin_lt10(self):
        ...
        self.data.loc[(self.data.blood_pressure_min<10) & (self.data.blood_pressure_max>100), 'blood_pressure_min'] = np.nan
