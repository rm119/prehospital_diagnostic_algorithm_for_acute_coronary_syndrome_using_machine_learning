...
def tuning_logistic_regression(X, y, n_trials=N_TRIALS):
    ...
    for solver in list('012'):
        ...
        study.optimize(objective_variable_solver(
            solver, X, y), n_trials=n_trials)
        final_trials.append(study)

    for i in range(len(final_trials)):
        print(
            f'Logistic ver.0{i}', final_trials[i].best_trial.params, final_trials[i].best_trial.value)
    ...
    if estimatorname == 'SVC':
        ...
        if params['booster'] == 'dart':
            ...
            params['rate_drop'] = trial.suggest_float(
                'rate_drop', 1e-3, 1.0, log=True)
            params['skip_drop'] = trial.suggest_float(
                'skip_drop', 1e-3, 1.0, log=True)
