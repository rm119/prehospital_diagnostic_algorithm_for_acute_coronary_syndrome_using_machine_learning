...
def nestcv_evaluation(target):
    ...
    for features in [EXPLANATORY_VARIABLES, SELECTED_VARIABLES]:
        ...
        if n_features == 43:
            ...
            drop_cols = list(set(EXPLANATORY_VARIABLES)
        ...
        for estimatorname in estimators:
            ...
            utils.save_data(
                models[estimatorname], f'models/modelevaluation_{target}_{n_features}_features_{estimatorname}.pkl')
...
def generate_finalmodel(target):
    ...
    for features in [EXPLANATORY_VARIABLES, SELECTED_VARIABLES]:
        ...
        for estimatorname in estimators:
            results = iteration.parameter_tuning(
                estimatorname, X, y, target=target, drop_cols=drop_cols, Tuning=True)
            imputator = results['imputator']
            if drop_cols:
               # X_imputed = imputator.fit_transform(X).drop(columns=drop_cols)
                X_ext_imputed = imputator.transform(
                    X_ext).drop(columns=drop_cols)
            else:
        ...
        utils.save_data(
            results, f'models/finalmodel_{target}_{n_features}_features_{estimatorname}.pkl')
...
def generate_finalmodel_voting(target):
    ...
    for features in [EXPLANATORY_VARIABLES, SELECTED_VARIABLES]:
        ...
        for submodelname in estimators:
            ...
            submodels.append(
                ('pipeline_' + submodelname, submodel_info['model']))
        ...
        results['threshold'] = evaluation.calc_gmeans(y, clf.predict_proba(X_imputed)[
                                                      :, 1], thresh=None)['threshold']
        ...
        utils.save_data(
            cv_score, f'models/modelevaluation_{target}_{n_features}_features_{estimatorname}.pkl')
...
def print_ncv_scores(target, n_features):
    ...
    for estimatorname in estimators + ['VotingClassifierAll']:
        models[estimatorname] = utils.load_data(
            f'models/modelevaluation_{target}_{n_features}_features_{estimatorname}.pkl')
        ...
        for label in ['train', 'val']:
            ...
            df_score_mean = df_score_mean.append(
                evalscore.loc['mean'].rename(estimatorname))
            df_score_std = df_score_std.append(
                evalscore.loc['std'].rename(estimatorname))

        print(df_score_mean[['ROC AUC', 'Accuracy',
                             'Sensitivity', 'Specificity', 'F1', 'PPV', 'NPV']])
        print(df_score_std[['ROC AUC', 'Accuracy',
                            'Sensitivity', 'Specificity', 'F1', 'PPV', 'NPV']])
